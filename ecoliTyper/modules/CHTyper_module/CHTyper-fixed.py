...
if args.db_path is None or args.db_path == '':
   ...
   db_config_file = os.path.join(args.db_path, 'config')
...
if not os.path.exists(args.out_path):
   sys.exit("Input Error: Output directory does not exist!\n")
else:
   out_path = args.out_path

# Check if valid path to BLAST is provided
if not os.path.exists(args.blast_path):
   sys.exit("Input Error: The path to BLAST does not exist!\n")
else:
   blast = args.blast_path

# Check if databases and config file are correct/correspond
if args.databases is None or args.databases == '':
...
with open(db_config_file) as f:
   for l in f:
      ...
      for ext in extensions:
         db = os.path.join(db_path, f"{db_prefix}.{ext}")
...
if args.databases is None:
   ...
   # Use only specified databases
   db_list = []
   for db_prefix in args.databases:
      if db_prefix in dbs:
         db_list.append(db_prefix)
      else:
         sys.exit("Input Error: Provided database was not recognised! (%s)\n"%db_prefix)

# Calling blast and parsing output
results, query_align, homo_align, sbjct_align = Blaster(inputfile, db_list,
                                                        db_path, out_path,
                                                        min_cov, threshold, blast)
# Making output files
tab_file = open(os.path.join(out_path, "results_tab.txt"), 'w')
table_file = open(os.path.join(out_path, "results_table.txt"), 'w')
txt_file = open(os.path.join(out_path, "results.txt"), 'w')
ref_file = open(os.path.join(out_path, "Reference_gene_seq.fsa"), 'w')
hit_file = open(os.path.join(out_path, "Hit_in_genome_seq.fsa"), 'w')
...
for db in db_list:
   ...
   if results[db] == "No hit found":
      ...
      for hit in results[db]:
         ...
         # Saving the output to print the txt result file alignments
