...
Date: 2026-07-23 (Added dynamic minID/minCov thresholds)
...
class EcoliAbricateExecutor:
    ...
    def __init__(self, cpus: int = None, min_identity: int = 80, min_coverage: int = 80):
        # Setup logging FIRST
    ...
    def run_abricate_single_db(self, genome_file: str, database: str, output_dir: str) -> Dict[str, Any]:
        """Run ABRicate on a single genome with specific database using configured thresholds"""
        ...
        cmd = [
            ...
            genome_file, 
            '--db', database,
            '--minid', str(self.min_identity),
            '--mincov', str(self.min_coverage)
        ]
        
        self.logger.info("Running ABRicate: %s --db %s (minid=%d%%, mincov=%d%%)", 
                        genome_name, database, self.min_identity, self.min_coverage)
...
{self._get_ascii_art()}
            ...
            <p style="color: #666; font-size: 0.9em;">Thresholds: minID={self.min_identity}%, minCov={self.min_coverage}%</p>
        </div>
    ...
    def _create_database_json_summary(self, database: str, hits: List[Dict], output_base: str):
        ...
        json_report = {
            "metadata": {
                ...
                "thresholds": {
                    "minimum_identity": self.min_identity,
                    "minimum_coverage": self.min_coverage
                }
            },
            "summary_statistics": {
...
"""
    ...
    def process_multiple_genomes(self, genome_pattern: str, output_base: str = "ecoli_abricate_results") -> Dict[str, Any]:
        ...
        print(f"   Thresholds: minID={self.min_identity}%, minCov={self.min_coverage}%")
        print("="*80)
...
Examples:
  ...
  # Adjust minimum identity and coverage thresholds (default: 80%)
  python ecoli_abricate.py "*.fna" --minid 90 --mincov 85
...
Supported FASTA extensions: .fasta, .fa, .fna, .faa
    ...
    parser.add_argument('--minid', type=int, default=80,
                       help='Minimum identity threshold for ABRicate (default: 80)')
    parser.add_argument('--mincov', type=int, default=80,
                       help='Minimum coverage threshold for ABRicate (default: 80)')
    
    args = parser.parse_args()
    
    # Validate thresholds
    if not 0 <= args.minid <= 100:
        parser.error("--minid must be between 0 and 100")
    if not 0 <= args.mincov <= 100:
        parser.error("--mincov must be between 0 and 100")
    
    executor = EcoliAbricateExecutor(cpus=args.cpus, min_identity=args.minid, min_coverage=args.mincov)
    ...
    try:
        ...
        executor.logger.info("Thresholds: minID=%d%%, minCov=%d%%", executor.min_identity, executor.min_coverage)
